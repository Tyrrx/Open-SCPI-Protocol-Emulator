...
namespace Domain
{
    public static class ConfigureVoltage
    {
        ...
        {
            electricalMeasuringSubsystem.ElectricalUnitOfMeasureBehaviorSubject.OnNext(ElectricalUnitOfMeasure.Voltage);
            ...
            electricalMeasuringSubsystem.RangeBehaviorSubject.OnNext(range.Match(
                some => some,
                () => Range.Auto));
            electricalMeasuringSubsystem.ResolutionBehaviorSubject.OnNext(resolution.Match(some => some,
                () => Resolution.Def));
